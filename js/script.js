            ...
            <div id="recipe-${recipe.id}" class="col s12 m6 l4">
              <div class="card">
                <div class="card-image">
                  ...
                  <a class="btn-floating halfway-fab waves-effect waves-light" onclick="heartFavourites('${
                      recipe.id
                  }')">
                    <i class="material-icons fav heart ${
                        isFavourite ? 'is-liked' : ''
                    }">favorite</i>
                  </a>
                </div>
                ...
                <p class="time">${recipe.data().time}'</p>
                  ...
                  <a class="waves-effect waves-light btn radius" onclick="openRecipe('${
                      recipe.id
                  }')">OPEN RECIPE</a>
                </div>
              </div>
...
function openRecipe(id) {
  ...
  <div class="list" id="recipe-${recipe.id}">
    ...
    <div class="card-image" id="recipe-card-img">
      <img src="${recipe.data().img}">
      <p class="time">${recipe.data().time}'</p>
      <a class="btn-floating halfway-fab waves-effect waves-light recipe-floating-button " onclick="heartFavourites('${
          recipe.id
      }')">
  ...
  <a class="waves-effect waves-light btn radius" onclick="toast()">I'M DONE</a>
  </div>
  `;
...
function toast() {
    ...
    <h1>YOU MADE IT!</h1>
    <span class="again-center"> You've used the ingredients in your fridge. No more food waste!</span>
    </div>`
    });
}
// FAVOURITES ==================================================================
...
function checkFavourites() {
    usersRef.doc(userData.uid).onSnapshot(function(snapshotData) {
        ...
        // Check if the user record exists & contains favourites
        if (data && data.favourites) {
            let myFavs = data.favourites;
            favouriteRecipes = myFavs;

            if (myFavs.length === 0) {
                document.querySelector(
                    '#favourites-recipes-container'
                ).innerHTML = '';
            }
...
function appendFavourites(selectedRecipes) {
  ...
  <div class="col s12 m6 l4" id="recipe-${recipe.id}">
    <div class="card">
      <div class="card-image">
        <img src="${recipe.data().img}">
        <a class="btn-floating halfway-fab waves-effect waves-light" onclick="deleteFavourites('${
            recipe.id
        }')">
...
function heartFavourites(id) {
    ...
    usersRef
        ...
        .then(function(userDoc) {
            ...
            if (data && data.favourites && data.favourites.includes(id)) {
                deleteFavourites(id);
            } else {
                addFavourite(id);
            }
            ...
            uploadTask.snapshot.ref
                ...
                .then(function(downloadURL) {
                    ...
                    usersRef
                        ...
                        .set(
                            {
                                myRecipes: firebase.firestore.FieldValue.arrayUnion(
                                    newRecipe
                                )
                            },
                            {
...
//APPEND NEW RECIPE ============================================================
// check all favourites after log in
function checkMyRecipes() {
    usersRef.doc(userData.uid).onSnapshot(function(snapshotData) {
        const data = snapshotData.data();

        // Check if the user record exists & contains myRecipes
        if (data && data.myRecipes) {
            let myRecipes = data.myRecipes;
            mySavedRecipes = myRecipes;
            console.log(mySavedRecipes);

            if (myRecipes.length === 0) {
                document.querySelector('#myRecipes-container').innerHTML = '';
            }
            appendNewRecipe(myRecipes);
        }
    });
    // console.log(newRecipe);
}

function appendNewRecipe(newRecipe) {
    console.log(newRecipe);
    let htmlTemplate = '';

    for (let recipe of newRecipe) {
        htmlTemplate += `
    <div class="col s12 m6 l4" id="recipe-${recipe.id}">
      <div class="card">
        <div class="card-image">
        ...
        <div class="card-content">
          ...
          <a class="waves-effect waves-light btn radius" onclick="">OPEN RECIPE</a>
        </div>
      </div>
    </div>
  `;
...
// function openMyRecipe() {
//   usersRef.doc(userData.uid).onSnapshot(function(snapshotData) {
//     let myRecipes = snapshotData.data().myRecipes;
//     mySavedRecipes = myRecipes;
//     console.log(mySavedRecipes);
//
//   });
//   console.log(newRecipe);
//   // const recipe = usersRef.id.myRecipes;
...
//   // loop throught all ingredients
//   let ingredientsTemplate = "";
//   for (let ingredient of mySavedRecipes) {
//     ingredientsTemplate += `
//     <li class="recipe-ingredient">
//         <label>
//           <input type="checkbox" />
//         <span>${ingredient.amount + " " + ingredient.title}</span>
//         </label>
//     </li>`;
//   }
//   // loop throught all steps
//
//   let stepsTemplate = "";
//   for (let step of mySavedRecipes) {
//     stepsTemplate += `
//     <p class="recipe-step">
//     <label>
//     <input type="checkbox" />
...
//   console.log(mySavedRecipes);
//
//   for (let recipe of mySavedRecipes) {
//     htmlTemplate = `
//   <div class="list" id="recipe-${recipe.id}">
